...
const GetProfile = async (req, res) => {
  ...
  try {
    ...
    return res
      ...
      .json({
        success: false,
        message: "something when wrong while extracting company",
        error: error,
      });
  }
  ...
  let existUsers;
  try {
    existUsers = await user.find({ id_entreprise: id });
  } catch (error) {
    return res.status(500).json({
      success: false,
      message: "something when wrong while extracting users",
      error: error,
    });
  }
  ...
  return res
    .status(200)
    .json({ success: true, message: "success", companyData: existCompany, userData: existUsers });
